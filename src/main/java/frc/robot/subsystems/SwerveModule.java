...
/**
 * Represents a Swerve Module on the robot. Contains movement and turning
...
public class SwerveModule {
     ...
     * 
     * @param config The configuration for this module. This is how the SwerveModule
     *               gets its constants (Motor IDs, offsets, etc.)
     */
    public SwerveModule(SwerveModuleConfigurations config) {
        ...
        // This makes getPosition() on the turning motor encoder give it's rotation in
        // radians.
        turningMotor.getEncoder().setPositionConversionFactor(SwerveDriveConstants.TURNING_ENCODER_TO_RAD);
        // This makes getVelocity() on the turning motor encoder give its velocity in
        // radians/second.
    ...
    public double getTurningPosition() {
        ...
        boolean sign = pos < 0;
        // SmartDashboard.putNumber("posBefore "+config.name(),
        // Units.radiansToDegrees(pos));
        pos = Math.abs(pos);
        pos = pos % (2 * Math.PI);
        if (sign)
            pos = (2 * Math.PI) - pos;
    ...
    /**
     * Gives the value from the absolute encoder (CANCoder). Will always keep its
     * angle even if the robot is turned off.
     * 
    ...
    public double getDriveVelocity() {
        return driveMotor.getSelectedSensorVelocity()
                * SwerveDriveConstants.DRIVE_ENCODER_VELOCITY_TO_METERS_PER_SECOND;
    }

    /**
     * Gives the current {@code SwerveModuleState} of the module. Contains the
     * current angle and velocity of the wheel.
     */
    ...
    /**
     * Gives the current {@code SwerveModulePosition} of the module. Contains the
     * total distance travelled and the current angle of the wheel.
     */
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                driveMotor.getSelectedSensorPosition() * SwerveDriveConstants.DRIVE_ENCODER_TO_METERS,
     ...
     * 
     * @param state The desired {@code SwerveModuleState}. Contains the desired
     *              velocity and angle of the wheel.
     */
    public void setDesiredState(SwerveModuleState state) {
        ...
        // This will minimize the change in angle by allowing the wheel to spin
        // backwards if needed.
        // For example, if you want to make the robot move in the opposite direction,
        // this will make it so the wheel will simply spin backwards instead of turning
        // the whole wheel around.
        state = SwerveModuleState.optimize(state, getState().angle);

        // Calculates the (estimated) power needed for the wheel to reach the speed
        // needed.
